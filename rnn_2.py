...
def run_lstm_model(tensorflow_data,data_,train=True,test=False, batch_size=72, epoch=20):

    sess,minimize,error,data,target,accuracy_class_1,accuracy_class_2,accuracy_class_3 = tensorflow_data
    train_input,train_output,val_input,val_output,test_input,test_output = data_
    ...
    no_of_batches = int(train_input.shape[1]) / batch_size
...
def main(params,data,train=True):
    ...
    print '\n\ntrain_data shape: ',np.array(train_input).shape,np.array(train_output).shape
    print 'val_data shape: ',np.array(val_input).shape,np.array(val_output).shape
    print 'test_data shape: ',np.array(test_input).shape,np.array(test_output).shape
    ...
    if train:
        run_lstm_model(tensorflow_data,data,test =params['test'], batch_size=batch_size,epoch=epoch)
    run_lstm_model(tensorflow_data,data,train = False,test = params['test'])
